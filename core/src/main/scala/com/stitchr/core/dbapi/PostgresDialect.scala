...
/**
 * NH: had to bypass shortType... on metadata columns... Not sure why
 *
 */
...
class PostgresDialect extends JdbcDialect {
  ...
  override def getCatalystType(sqlType: Int, typeName: String, size: Int, md: MetadataBuilder): Option[DataType] =
    if (sqlType == Types.REAL) {
      ...
      Some(IntegerType) // (Some(ShortType)
  ...
  private def toCatalystType(typeName: String, precision: Int, scale: Int): Option[DataType] = typeName match {
    ...
    case "int2"             => Some(IntegerType) // Some(ShortType)
    ...
    case "numeric" | "decimal"                           => Some(FloatType) // NH Some(DecimalType.bounded(precision, scale))
    case _                                               => None
  }
  ...
  override def getJDBCType(dt: DataType): Option[JdbcType] = dt match {
    ...
    /* NH testing  case ArrayType(et, _) if et.isInstanceOf[AtomicType] =>
   ...
   */
  /* compile throws an error that nothing to override!! not sure why
  // def getTruncateQuery(table : scala.Predef.String, cascade : scala.Option[scala.Boolean] = { /* compiled code */ }) : scala.Predef.String = { /* compiled code */ }
