...
case class SparkJdbcImpl(jdbcProps: JdbcProps) extends SparkJdbc {
  ...
  override def readDF(query: String): DataFrame =
    // spark.read.option("fetchsize", com.stitchr.core.common.EnvConfig.props.getInt("jdbc.fetchsize")).jdbc(url, s"( $query ) as t", connectionProperties)
  ...
  override def readDF(query: String, partitionKey: String = "key", numberOfPartitions: Int = 2): DataFrame =
    if (numberOfPartitions == 1) readDF(query)
      ...
      spark.read
        .option("fetchsize", jdbcProps.fetchsize) // com.stitchr.core.common.EnvConfig.props.getInt("jdbc.fetchsize"))
        .jdbc(
            url = url,
  ...
  def writeTable(dataframe: DataFrame, tableName: String): Unit =
    ...
    println(s"needs to be implemented")
